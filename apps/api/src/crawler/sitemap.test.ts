 ...
 *
 * @param responses - Per-URL mocked sitemap responses.
 */
function stubNetwork(
  responses: Partial<Record<string, { xml?: string; status?: number; contentType?: string }>>,
): void {
  mockPoliteFetch.mockImplementation((url: string) => {
    const response = responses[url];
    const status = response?.status ?? (response?.xml !== undefined ? 200 : 404);
    const body =
      status === 200 && response?.xml !== undefined
        ? ({
            __xml: response.xml,
            pipeThrough: vi.fn(() => ({ __xml: response.xml })),
  ...
  mockReadBodyText.mockImplementation((body: ReadableStream<Uint8Array>) =>
    Promise.resolve((body as unknown as { __xml: string }).__xml),
  );
}
...
describe("prioritizeShallow", () => {
  it("sorts shallow paths first and caps the result", () => {
    ...
    expect(prioritizeShallow(entries, 3).map((entry) => entry.url)).toEqual([
      "https://example.com/",
      "https://example.com/a",
...
describe("discoverSitemapEntries", () => {
  ...
  it("falls back to conventional /sitemap.xml when the declared sitemap 404s", async () => {
    ...
    expect(result.entries.map((entry) => entry.url)).toContain("https://example.com/");
  ...
  it("recurses through a declared sitemap index", async () => {
    ...
    expect(result.entries.map((entry) => entry.url).sort()).toEqual([
...
<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
      ${Array.from({ length: 8 }, (_, index) => `<sitemap><loc>https://stripe.com/sitemap/partition-${String(index)}.xml</loc></sitemap>`).join("")}
</sitemapindex>`;
    stubNetwork({
      "https://stripe.com/sitemap/sitemap.xml": { xml: stripeIndex },
      ...Object.fromEntries(
        Array.from({ length: 8 }, (_, index) => [
          `https://stripe.com/sitemap/partition-${String(index)}.xml`,
          {
            xml: `<urlset><url><loc>https://stripe.com/page-${String(index)}</loc></url></urlset>`,
          },
        ]),
      ),
    });
    ...
    expect(result.entries.map((entry) => entry.url)).toEqual(
      Array.from({ length: 8 }, (_, index) => `https://stripe.com/page-${String(index)}`),
    );
